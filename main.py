...
pool = psycopg2.pool.SimpleConnectionPool(1, 50, dbname='employee', port='5432',
                                          user='postgres', password='abc', host='')
...
def selectnodes():
    ...
    #for rm_condition in rmrules.selectrmrule():
        ...
        #               data.nodes p1 tablesample system_rows(10)
        #               ,data.edges e
        #               where p1.geom in
        #               (select geom from data.nodes order by geom <-> (p1.geom)
        #               limit 10) and p1.uid = e.uid1
...
def showheatmap(dict1, dict2):
    ...
    if(len(f1)>1 and len(f2)>1 and len(f3)>1):
...
def moveAndDisplay():
    ...
    for i in range(1, tmp):
        t = threading.Thread(target=display, args=(qap,score,rmp,var2,q))
        t.setDaemon(True)
        threadlist.append(t)
...
def display(qap,score,rmp,var2,q):
    ...
    for elem in selectnodes():
                ...
                if ((qap).count(', (' + str(key) + ',') > 0):
                    if (score > 0):
                        if (key in qval and qval[key] != score):
                            print(str(key) + ' quality score changed from ..' + str(qval[key]) + ' to ' + str(score))

                        else:
                            print(str(key) + ' quality score assigned to new nodes ..' + str(score))
                    ...
                    if depth[len(depth) - 1][0] == 3:
                        ...
                        if(tmpval>0):
                            if (key in qval and qval[key] != tmpval):
                                print(str(key) + ' quality score changed from ..' + str(qval[key]) + ' to ' + str(tmpval))
                            else:
                                print(str(key) + ' quality score assigned to new nodes ..' + str(tmpval))
                            ...
                            if ((elem != '') and (str(points.keys())).count(', ' + elem + ',') > 0):
                                ...
                                if (levenshteinDistance(tmp_list2[:-2], tmp_list3[:-2]) < 25):
                                    ...
                                    if newp not in points.values():
                                        if (key in points):
                                            print(str(key) + ' position changed from ..' + str(
                                                points[key]) + ' to ' + str(newp))
                                        else:
                                            print(str(key) + ' position assigned to new nodes ..' + str(newp))
                                    ...
                                    if ((elem != '') and (str(points.keys())).count(', ' + elem + ',') > 0):
                                        ...
                                        if (newp not in points.values()):
                                            if (key in points):
                                                ...
                                                print(str(key) + ' position assigned to new node ..' + str(newp))
        ...
        if (indicator):
            ...
            print("-------------------------")
            print("number of nodes on screen " + str(len(qval)))
            #print("modified points " + str(points))
            #print("modified quality score " + str(qval))
            print("-------------------------")
    return
...
def altercall(ex, q):
    ...
    for condition in sqlquerconcat():
        ...
        if (int(list(tmpcount)[0][0]) == 0):
            #cons = genkeyvalfromrule(condition)
...
def quercheck(condition):
    ...
    tmp = condition.split("*")
    #print(str(condition[0])+ "..split to.."+str(tmp))
    tmp_str = tmp[1]
    ...
    for rec in querlhs(condition):
        ...
        for elem in tmp_z:
            if ('NULL' not in elem):
                ...
                #print(test + " compared with " + strkey)
...
def computeRank(p,cons,q):
     ...
     for key in cons:
         #print(str(rule) + "..checked in.." + str(key))
