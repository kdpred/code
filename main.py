...
imgpath = input()
#imgpath = 'C:\\Users\\kuldeep\\OneDrive\\Desktop\\'
...
def getschema():
    ...
    sch = [desc[0] for desc in cur.description]
...
def sqlAGC():
    ...
    sqlQuery_addGeoColumn = """ alter table data.nodes add column if not exists geopoint POINT;"""
...
def sqlAGP():
    ...
    sqlQuery_agp = """ update data.nodes set geopoint = point(random()*(180 - (-180)-1)+(-180), random()*(180 - (-180)-1)+(-180));"""
...
def sqlAGD():
    ...
    sqlQuery_agp = "alter table data.nodes add column if not exists geom geometry(POINT,4326);"
...
def sqlgeos():
    ...
    sqlQuery_agp = "create index if not exists newgeoidx on data.nodes using gist (geom);"
    ...
    sqlQuery_agp = "create index if not exists newgeoidx on data.nodes using gist (geopoint);"
...
def showheatmap(dict1, dict2):
    ...
    if(len(f1)>7 and len(f2)>7 and len(f3)>7):
        ...
        axe.set_zlim(0, +1)
        plt.savefig(imgpath + '\\dcimgs\\' + str(len(qval)) + '.png')
...
def moveAndDisplay():
    ...
    img1 = []
    imglist = list()
    dirFiles = os.listdir(imgpath + "dcimgs\\")
    dirFiles.sort(key=lambda f: int(re.sub('\D', '', f)))
    for file in dirFiles:
        if switch == 0:
            img1 = Image.open(imgpath + "dcimgs\\" + file)
            switch = 1
        imglist.append(Image.open(imgpath + "dcimgs\\" + file))

    img1.save(imgpath + "dcgif.gif", save_all=True, append_images=imglist, duration=350,
              loop=0)

    os.system(imgpath + "dcgif.gif")
...
def quercnt1(quer):
    ...
    query = getkeyvals(quer[0])

    ind = True
    for elem in query:
        if elem not in schema:
            ind = False

    if (quer[0].count('=")') == 0) and ind:
...
def run():
    ...
    if q.empty() == False:
        for elem in q.queue:
            ...
            if len(rhsval) > 0:
                #print(sqlquery)
                if sqlquery.count(')') == sqlquery.count('(') and (str(lhs)).count("=')") == 0 \
                        and (str(lhs)).count('=")') == 0 and (str(rhsval)).count("=')") == 0 \
                        and (str(rhsval)).count('=")') == 0 and (str(rhskey)).count("=')") == 0 \
                        and (str(rhskey)).count('=")') == 0:
                    # print(sqlquery)
